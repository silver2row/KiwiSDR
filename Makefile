...
INT_FLAGS += -DBUILD_DIR=STRINGIFY\($(BUILD_DIR)\)
INT_FLAGS += -DREPO_NAME=STRINGIFY\($(REPO_NAME)\) -DREPO_GIT=STRINGIFY\($(REPO_GIT)\) -DREPO_DIR=STRINGIFY\($(REPO_DIR)\)
...
#	    find . -name "*.min.js" -size -1k -exec rm "{}" \;
	    ...
	    install -D -o root -g root $(DIR_FILE_SRC)/pkgs/frp/$(ARCH_DIR)/frpc /usr/local/bin/frpc
